...
export default function QRPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (citaId) {
        ...
        await loadCitaAndGenerateQR(citaId);
      } else {
        // Modo escanear QR
        setMode('scan');
      }
  ...
  const loadCitaAndGenerateQR = async (citaId: string) => {
    try {
  ...
  const handleScan = (detectedCodes: any[]) => {
    if (detectedCodes && detectedCodes.length > 0 && !processing) {
  ...
  const handleScanError = (err: any) => {
    ...
    setCameraError("Error accediendo a la cámara. Verifica los permisos.");
    setCameraActive(false);
  };
  ...
  const processQRCode = async (qrData: string) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        setSuccess(true);
        setError(null);
        showSuccess("Éxito", "Check-in registrado exitosamente");
        setTimeout(() => resetScanner(), 3000);
      } else {
        setError(data.message || "Error procesando el código QR");
        setSuccess(false);
        showError("Error", data.message || "Error procesando el código QR");
      }
    } catch (err: any) {
      console.error("Error procesando QR:", err);
      setError("Error al procesar el código QR. Intenta nuevamente.");
      setSuccess(false);
      showError("Error", "Error al procesar el código QR");
    } finally {
      setProcessing(false);
    }
  };
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-center">
                    {cameraActive ? (
                      <div className="w-full space-y-3">
                        <div className="relative overflow-hidden rounded-xl shadow-lg border-2 border-[#16A34A] h-64">
                          <Scanner
                            onScan={handleScan}
                  ...
                  <div className="bg-gray-50 rounded-lg p-2.5 border border-gray-200">
                    <div className="flex items-center gap-2 justify-center">
                      <ScanLine className={`h-3.5 w-3.5 ${cameraActive ? 'text-[#16A34A] animate-pulse' : 'text-gray-400'}`} />
                      <p className="text-xs font-medium text-gray-600">
                        {cameraActive ? "Escaneando..." : "Presiona para activar"}
                      </p>
                    </div>
                  </div>
