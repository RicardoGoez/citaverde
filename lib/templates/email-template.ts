 ...
 */
export function getVerificacionEmailTemplate(data: {
  nombre: string;
  email: string;
  verifyLink: string;
  loginLink?: string;
  ...
  const content = `
    <p style="margin: 10px 0; font-size: 16px; color: #111827;">Hola <strong>${data.nombre}</strong>,</p>
    <p style="margin: 10px 0; font-size: 16px; color: #111827;">Gracias por registrarte en CitaVerde. Para completar tu registro, necesitamos verificar tu dirección de email.</p>
    ...
    <div style="background: #eff6ff; border-left: 4px solid #3b82f6; padding: 20px; margin: 20px 0; border-radius: 8px;">
      <p style="margin: 0 0 15px 0; color: #1e40af; font-size: 16px; font-weight: 600;">
        📧 Verifica tu email: ${data.email}
      </p>
      <p style="margin: 0; color: #1e40af; font-size: 14px;">
        Haz clic en el botón de abajo para verificar tu dirección de email.
      </p>
    </div>

    <div style="background: #fef3c7; border-left: 4px solid #f59e0b; padding: 15px; margin: 20px 0; border-radius: 8px;">
      <p style="margin: 0; color: #92400e; font-size: 14px;">
        ⚠️ <strong>Importante:</strong> Este enlace expirará en 7 días. Si no verificas tu email, no podrás iniciar sesión.
      </p>
    </div>

    <p style="margin: 20px 0 10px 0; font-size: 16px; color: #111827;">Si no solicitaste esta cuenta, puedes ignorar este mensaje.</p>
  `;

  return generateEmailTemplate({
    title: "Verifica tu Email - CitaVerde",
    content,
    actions,
    footer: "Si tienes problemas con el botón, copia y pega este enlace en tu navegador: " + data.verifyLink
  });
}
