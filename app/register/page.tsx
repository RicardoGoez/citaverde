...
export default function RegisterPage() {
  ...
  const validatePhoneColombia = (phone: string): boolean => {
    // Formato: +57 300 1234567 o 3001234567 o 300 123 4567
    const phoneRegex = /^(\+57\s?)?(3\d{2}[\s\-]?\d{3}[\s\-]?\d{4}|3\d{9})$/;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.phone && !validatePhoneColombia(formData.phone)) {
      setError("Ingresa un número de celular válido de Colombia (ej: +57 300 1234567)");
      return;
    }
    ...
    try {
      ...
      if (user) {
        ...
        try {
          ...
          await NotificationService.notifyCuentaCreada(user.id, {
        ...
        success("Registro exitoso", "Cuenta creada correctamente. Revisa tu email con tus credenciales");
          ...
          <CardContent>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-[#64748b]">Opcional: Formato +57 300 1234567</p>
              </div>
