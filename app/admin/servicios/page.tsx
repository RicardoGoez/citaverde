...
import { getServicios, getSedes, createServicio, updateServicio, deleteServicio } from "@/lib/actions/database";
...
export default function ServiciosPage() {
  ...
  const handleDelete = async (servicio: any) => {
    // Verificar permiso
    const tienePermiso = await hasPermAsync('servicios', 'eliminar');
    if (!tienePermiso) {
      error("Sin permisos", "No tienes permiso para eliminar servicios. Este permiso puede haber sido removido.");
      return;
    }
  ...
  const handleSubmit = async () => {
    // Verificar permisos
    if (isEditing) {
      const tienePermiso = await hasPermAsync('servicios', 'editar');
      if (!tienePermiso) {
        error("Sin permisos", "No tienes permiso para editar servicios. Este permiso puede haber sido removido.");
        return;
      }
    } else {
      const tienePermiso = await hasPermAsync('servicios', 'crear');
      if (!tienePermiso) {
        error("Sin permisos", "No tienes permiso para crear servicios. Este permiso puede haber sido removido.");
        return;
      }
    }
    ...
    try {
      if (isEditing) {
        await updateServicio(editingServicio.id, {
          ...formData,
          duration: 30, // Valor por defecto si no existe
        });
        // Recargar datos desde la BD
        const serviciosData = await getServicios(sedeSeleccionada?.id);
        setServicios(serviciosData);
