...
import { getCitas, createCita, getServicios, getProfesionales, updateCita, deleteCita, getHorariosEspeciales, getDisponibilidades } from "@/lib/actions/database";
...
export default function CitasRecepcionista() {
  ...
  const isFechaValida = (fechaStr: string): boolean => {
    ...
    if (formData.profesional && disponibilidadesDoctor.length > 0) {
      // Convertir día de la semana: 0=Dom, 1=Lun, ..., 6=Sáb
      // La BD usa: 1=Lun, 2=Mar, ..., 5=Vie, 7=Domingo (a veces)
      const diaSemanaBD = diaSemana === 0 ? 7 : diaSemana;
      const tieneDisponibilidad = disponibilidadesDoctor.some((disp: any) => {
        if (disp.tipo === 'jornada' && disp.recurrente) {
          return disp.dia_semana === diaSemanaBD || disp.dia_semana === diaSemana;
        }
        // Para disponibilidades no recurrentes, verificar rango de fechas
        if (disp.tipo === 'jornada' && disp.fecha_inicio && disp.fecha_fin) {
          const fechaInicio = new Date(disp.fecha_inicio);
          const fechaFin = new Date(disp.fecha_fin);
          return fecha >= fechaInicio && fecha <= fechaFin && (disp.dia_semana === diaSemanaBD || disp.dia_semana === diaSemana);
        }
        return false;
      });
  ...
  const generarDiasDelCalendario = () => {
    ...
    for (let i = 1; i <= diasEnMes; i++) {
      ...
      const esValido = !esPasado && !esFinDeSemana && !esFestivo && tieneDisponibilidad;
  ...
  // Obtener horas disponibles según la disponibilidad del doctor
  const obtenerHorasDisponibles = (): string[] => {
    if (!formData.profesional || !formData.fecha || disponibilidadesDoctor.length === 0) {
    ...
    // Generar horas disponibles dentro del rango del doctor
    ...
    while (horaActual < finMinutos) {
      const horasNum = Math.floor(horaActual / 60);
      const minutos = horaActual % 60;
      const horaStr = `${String(horasNum).padStart(2, '0')}:${String(minutos).padStart(2, '0')}`;
    ...
    try {
      ...
      if (isEditing && editingCita) {
        ...
        const nuevaCita = await createCita({
          user_id: 'e0e0e0e0-0000-0000-0000-000000000003', // Usuario genérico del sistema
          servicio_id: formData.servicio,
          servicio: servicioSeleccionado?.name || '',
          profesional_id: formData.profesional || '',
          profesional: profesionalSeleccionado?.name || '',
          sede_id: sedeSeleccionada.id, // Usar la sede del recepcionista
          fecha: formData.fecha,
          hora: formData.hora,
      ...
      try {
        await NotificationService.notifyCitaConfirmada(
          ...
          'paciente@citavede.com' // Email del usuario genérico
        );
      } catch (emailError) {
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="mt-4">
              <Label>Fecha *</Label>
              <div className="border border-[#E5E7EB] rounded-lg overflow-hidden bg-white mt-1">
                          ...
                          className={`
                            ...
                            ${!dia.esValido
                              ...
                              : formData.fecha === dia.fechaStr
                                ...
                                : 'hover:bg-blue-50 hover:border-[#2563EB] border-2 border-transparent'
                            }
                            ${dia.esFestivo && 'bg-red-50 text-red-500'}
                          `}
                          title={dia.esFestivo ? 'Día festivo' : dia.esFinDeSemana ? 'Fin de semana' : dia.esPasado ? 'Fecha pasada' : !dia.tieneDisponibilidad ? 'Sin disponibilidad del doctor' : ''}
                        >
                          {dia.dia}
                        </button>
                      ) : (
                ...
                <div className="flex items-start gap-4 p-3 bg-gray-50 border-t border-[#E5E7EB] text-xs text-[#6B7280]">
                  <div className="flex items-center gap-2">
                    <div className="w-4 h-4 border-2 border-transparent bg-[#2563EB] rounded"></div>
              ...
              {formData.fecha && (
                <p className="text-xs text-gray-500 mt-1">
                  Fecha seleccionada: {new Date(formData.fecha).toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}
                </p>
              )}
            </div>
